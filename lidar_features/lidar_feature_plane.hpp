...
char cmd_ ;
template<typename T>
class LidarUnorgPlane : public LidarFeatureBase<T>
{
public:
	...
	LidarUnorgPlane(lcm::LCM * lcm,
		 ...
		 static void keyboardEventOccurred(const pcl::visualization::KeyboardEvent& event, void* nothing)
		 {
			...
			if(event.getKeySym() == "q" && event.keyDown()){
				std::cout << "skip this frame" << std::endl;
			}
		 }
		 ...
		 void load_params() 
		 {
			 ...
			 this->param_->getParam("min_normal_diff", min_normal_diff_, -0.1) ;
			 ...
			 this->param_->getParam("plane_num",plane_num_, 3);
			 this->param_->getParam("manual_select",manual_select_, false);
		 }
		 ...
		 void print_params() 
		 {
			 ...
			 printf("\tmin_normal_diff %20.3f\n",min_normal_diff_) ; 
			 printf("\tmax_search_radius %20.3f\n",max_search_radius_) ; 
			 printf("\tmax_plane_points %20d\n",max_plane_points_) ; 
			 printf("\tshow_cloud_orig %20.3d\n",show_cloud_orig_) ; 
			 printf("\tshow %20.3d\n",show_) ; 
			 printf("\tincorrect_normal_ratio %20.3d\n",incorrect_normal_ratio_);
			 printf("\tplane_num %20.3d\n",plane_num_);
			 printf("\manual_select %20.3d\n",manual_select_);
		 }
...
// 				viewer.spinOnce();	
			      ...
			      int arg=0) override
		{
			...
			while(true)
			{
				...
				if(pc_inliers->size() < min_inliers_ratio_ * N)
					break ;
				/// add the new plane to output only if it is satisfactory
				bool good_plane = false ;
				good_plane = !parallel_plane(planes, *coeff) ;
				bool good_normals = !reject_trees(ori_pc, *pc_inliers, *coeff);
				if(good_plane && good_normals && manual_select_)
				{
					boost::shared_ptr<pcl::visualization::PCLVisualizer> viewer_seletction;					
					viewer_seletction.reset(new pcl::visualization::PCLVisualizer("select"));
					viewer_seletction->setBackgroundColor(0.3,0.3,0.3);
					viewer_seletction->setSize(840, 480);
					pcl::visualization::PointCloudColorHandlerCustom<T> pc_origcolor ( pc_orig, 100, 100, 100 );
					sprintf(cloud_name, "cloud_orig") ;
					viewer_seletction->addPointCloud<T>(pc_orig, pc_origcolor, cloud_name) ;	
					viewer_seletction->registerKeyboardCallback(LidarUnorgPlane::keyboardEventOccurred, (void*)viewer.get());
					sprintf(cloud_name, "cloud_%d", cnt) ;
					pcl::visualization::PointCloudColorHandlerCustom<T> pc_inlierscolor ( pc_orig, 255, 0, 0 );
					viewer_seletction->addPointCloud<T>(pc_inliers, pc_inlierscolor, cloud_name) ;
					
					while(!viewer_seletction->wasStopped())
					{
						viewer_seletction->spin();
					}
					
					if(cmd_ == 'b') //skip this frame
					{
					...
					else  // reject current planes
					{
						if(show_)
						{
							sprintf(cloud_name, "cloud_unselected_%d", cnt_bad) ;			
							viewer->add_cloud_to_viewer<T>(pc_inliers, color_unselected, cloud_name) ;
							viewer->spin_once();
						}
						++cnt_bad ;
						printf("plane %d is not selected \n", cnt) ;
					}
					cmd_ = ' ';
				}		
				if(!manual_select_)
				{					
					if(good_plane && good_normals)
					{
						down_sample(*pc_inliers, *coeff) ;
						for(auto &p : pc_inliers->points)
						{
			...
			std::sort(pc.points.begin(), pc.points.end(), LidarUnorgPlane::comparator_timestamp) ;
			if(show_)
				viewer->show() ;
		}
		...
		{
			std::sort(cloud.points.begin(), cloud.points.end(), LidarUnorgPlane::comparator_distance) ;
		...
		{
			std::sort(cloud.begin(), cloud.end(), LidarUnorgPlane::comparator_distance) ;
