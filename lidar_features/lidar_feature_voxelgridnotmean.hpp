...
#ifndef _ICP_ADAPTER_VOXELGRIDNOTMEAN_HPP_
#define _ICP_ADAPTER_VOXELGRIDNOTMEAN_HPP_
...
class VoxelGridNotMean : public pcl::VoxelGrid<PointT>
{
...
VoxelGridNotMean<PointT>::applyFilter (PointCloud &output)
{
	...
	for (unsigned int cp = 0; cp < first_and_last_indices_vector.size (); ++cp)
	{
		...
		float minpt = 10e10;
		Eigen::Vector4f realcentroid;
		PointT realpoint;
		
		for (unsigned int li = first_index; li < last_index; ++li)
		{
			
			Eigen::Vector4f tmppt = input_->points[index_vector[li].cloud_point_index].getVector4fMap ();
			if((centroid-tmppt).norm() < minpt)
			{
				minpt = (centroid-tmppt).norm();
				realcentroid = tmppt;
				realpoint = input_->points[index_vector[li].cloud_point_index];
			}
			
		}
		
		output.points[index] = realpoint;
